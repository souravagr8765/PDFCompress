...
6. **Iterative Processing**: For every target file:
...
7. **Retry Failed Uploads Pass**: After completing the main file iteration, query the database for all remaining `upload_failed` files and attempt a dedicated retry pass. Statistics are reported on success or persistent failure.
8. **Complete**: Print a total summary execution string containing processed counts, skipped counts, error events, and retry recovery rates. The global lock is released via a top-level try/finally block before script exit.
